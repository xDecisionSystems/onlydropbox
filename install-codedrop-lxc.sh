...
load_defaults_from_config_file() {
  local file="$1"
  [[ -f "$file" ]] || return 0

  INSTALL_DROPBOX="${INSTALL_DROPBOX:-$(read_config_value "INSTALL_DROPBOX" "$file")}"
  DROPBOX_USER="${DROPBOX_USER:-$(read_config_value "DROPBOX_USER" "$file")}"
  LOCAL_USER="${LOCAL_USER:-$(read_config_value "LOCAL_USER" "$file")}"
  PREFIX_PATH="${PREFIX_PATH:-$(read_config_value "PREFIX_PATH" "$file")}"
  SYNC_FOLDERS="${SYNC_FOLDERS:-$(read_config_value "SYNC_FOLDERS" "$file")}"
  INSTALL_CODE_SERVER="${INSTALL_CODE_SERVER:-$(read_config_value "INSTALL_CODE_SERVER" "$file")}"
  INSTALL_CLAUDE_CODE="${INSTALL_CLAUDE_CODE:-$(read_config_value "INSTALL_CLAUDE_CODE" "$file")}"
  INSTALL_CODEX_EXTENSION="${INSTALL_CODEX_EXTENSION:-$(read_config_value "INSTALL_CODEX_EXTENSION" "$file")}"
  INSTALL_PYTHON_EXTENSION="${INSTALL_PYTHON_EXTENSION:-$(read_config_value "INSTALL_PYTHON_EXTENSION" "$file")}"
}
...
load_initial_defaults() {
  local guess_user="${DROPBOX_USER:-${SUDO_USER:-}}"
  local guess_home
  local guess_file

  [[ -z "$guess_user" ]] && return 0
  if ! id -u "$guess_user" >/dev/null 2>&1; then
    return 0
  fi

  guess_home="$(resolve_user_home "$guess_user")"
  guess_file="$guess_home/.config/codedrop/codedrop.env"
  load_defaults_from_config_file "$guess_file"
}
...
INSTALL_CODE_SERVER=${INSTALL_CODE_SERVER:-n}
INSTALL_CLAUDE_CODE=${INSTALL_CLAUDE_CODE:-n}
INSTALL_CODEX_EXTENSION=${INSTALL_CODEX_EXTENSION:-n}
INSTALL_PYTHON_EXTENSION=${INSTALL_PYTHON_EXTENSION:-n}
EOF
"
}
...
if [[ -z "${LOCAL_USER:-}" || "$LOCAL_USER" == "root" ]]; then
  LOCAL_USER="$(prompt "LOCAL_USER (linux username for user-scoped install steps)" "${LOCAL_USER:-${SUDO_USER:-}}")"
...
refresh_runtime_paths
load_defaults_from_config_file "$ENV_FILE"
...
INSTALL_CODE_SERVER="${INSTALL_CODE_SERVER:-n}"
INSTALL_CODE_SERVER="$(normalize_yes_no_default "$INSTALL_CODE_SERVER")"
if prompt_yes_no "Install/keep code-server (system-wide)?" "${INSTALL_CODE_SERVER}"; then
...
INSTALL_CLAUDE_CODE="${INSTALL_CLAUDE_CODE:-n}"
INSTALL_CLAUDE_CODE="$(normalize_yes_no_default "$INSTALL_CLAUDE_CODE")"
if prompt_yes_no "Install Claude Code for user '$LOCAL_USER'?" "${INSTALL_CLAUDE_CODE}"; then
...
INSTALL_CODEX_EXTENSION="${INSTALL_CODEX_EXTENSION:-n}"
INSTALL_CODEX_EXTENSION="$(normalize_yes_no_default "$INSTALL_CODEX_EXTENSION")"
if prompt_yes_no "Install Codex extension in code-server for user '$LOCAL_USER'?" "${INSTALL_CODEX_EXTENSION}"; then
...
INSTALL_PYTHON_EXTENSION="${INSTALL_PYTHON_EXTENSION:-n}"
INSTALL_PYTHON_EXTENSION="$(normalize_yes_no_default "$INSTALL_PYTHON_EXTENSION")"
if prompt_yes_no "Enable Python support in code-server for user '$LOCAL_USER'?" "${INSTALL_PYTHON_EXTENSION}"; then
...
download_update_script_as_user
write_env_config
log "Saved installer defaults to $ENV_FILE"
